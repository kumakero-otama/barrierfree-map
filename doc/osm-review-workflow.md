...
- 記録地点をOpenStreetMap公式サイトで開く外部リンクを提供
- 管理者メモはPostgreSQLへ保存し、一覧の更新・再ログイン後も同じ記録へ再表示
- OSM公開前なら、保存済み生GPSを最新のOSM道路へ再マップマッチングし、変更案と管理画面の緑線を最新Way上の連続経路へ更新可能。この操作だけではOSMへ送信しない
