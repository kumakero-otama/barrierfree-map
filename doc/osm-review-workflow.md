...
点字ブロック記録を保存直後にOSMへ送らず、PostgreSQLの審査キューへ保存します。管理者がGoogleマップへの外部リンクで記録地点の航空写真を開き、管理画面のOSM上でGPS生座標、フィット経路、対象Wayと変更予定タグを確認します。承認した記録だけをStepBy専用OSMアカウントから送ります。
...
- 航空写真: 記録地点を中心にしたGoogleマップ（航空写真表示）を外部リンクで開く
- 管理画面内のOpenStreetMapにGPS生座標、保存経路、変更対象を重ねて表示
